...
public class MoneyUtil {
  ...
  //add by dmm 2013-12-2 start
    /**元转分*/
    public static BigDecimal Cent2Dollar(BigDecimal yuan) {
    	return new BigDecimal(MoneyUtil.Cent2Dollar(String.valueOf(yuan)));
	}
    /**分转元*/
    public static BigDecimal Dollar2Cent(BigDecimal cent) {
    	return new BigDecimal(MoneyUtil.Dollar2Cent(String.valueOf(cent)));
	}
	...
	//add by dmm 2013-12-2 end
	
    /**
     * 如果<code>money</code>为null,返回默认的货币值,0.00；
     * 否则调整货币，使之保留小数点后两位，并符合四舍五入。
     * @param money
     * @return
     */
    ...
    /**
     * ********************************************
     * method name   : getGoodsPriceComb 
     * description   : 格式化商品价格，格式为：30积分+3.00元
     * @return       : String
     * @param        : @param price 商品单价
     * @param        : @param points 单个商品所需积分
     * @param        : @param num 商品数量
     * @param        : @return
     * modified      : xuhuafeng ,  2017-4-12  上午11:06:55
     * *******************************************
     */
    public static String getGoodsPriceComb(BigDecimal price, BigDecimal points, Integer num, String paytype) {
    	BigDecimal number = new BigDecimal(num);
    	String totalPrice = String.valueOf(Cent2Dollar(multiply(price, number, 0)));
    	String totalPoints = String.valueOf(multiply(points, number, 0));
    	switch (paytype) {
		case Contants.PAYTYPE_POINTS:
			return totalPoints + "积分";
		case Contants.PAYTYPE_CASH:
			return totalPrice + "元";
		case Contants.PAYTYPE_COMB:
			return totalPoints + "积分+" + totalPrice + "元";
		default:
			return "价格待定";
		}
    }
