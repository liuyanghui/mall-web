 ...
 */
(function($) {
	mui.fn.find = function(selector) {
		return mui(this.selector +' '+ selector);
	};
	mui.fn.html = function(htmlStr) {
	...
	mui.fn.show = function() {
		this.each(function(index, element) {
			element.style.display = 'block';
		});
		return this;
	};
	mui.fn.hide = function() {
		this.each(function(index, element) {
			element.style.display = 'none';
		});
		return this;
	};
	mui.fn.tap = function(callback) {
		this.each(function(index, element) {
			element.addEventListener('tap', callback);
		});
		return this;
	};
	mui.fn.addClass = function(className) {
		this.each(function(index, element) {
			element.classList.add(className);
		});
		return this;
	};
	mui.fn.removeClass = function(className) {
		this.each(function(index, element) {
...
(function(obj) {
	obj.stringUtil = {
		...
		formatTime: function(time, format) {
			var o = {
				...
				'W': ['鏃�', '涓€', '浜�', '涓�', '鍥�', '浜�', '鍏�'][time.getDay()],
				'q+': Math.floor((time.getMonth() + 3) / 3)
			};
			if (format.indexOf('am/pm') >= 0) {
				format = format.replace('am/pm', (o['h+'] >= 12) ? '涓嬪崍': '涓婂崍');
				if (o['h+'] >= 12) o['h+'] -= 12;
			}
			if (/(y+)/.test(format)) {
	...
	obj.browser = (function() {
		var ua = navigator.userAgent.toLowerCase(), wx = !!ua.match(/micromessenger/i), br = sessionStorage.getItem('debug.browser') || '';
		if (!wx) {
			if (br == 'weixin') {
				wx = true;
			}
...
(function($) {
	...
	var stringUtil = {
		...
		getScriptTmpl: function($element, name) {
			return this.getQueryTmpl($element.find('script').html(), name);
		},
	...
	$.fn.template = function(data) {
		this.each(function(index, element) {
			var template = element.querySelector('script').innerHTML.trim();
			element.innerHTML = stringUtil.template(template, data);
		});
	};
})(mui);
